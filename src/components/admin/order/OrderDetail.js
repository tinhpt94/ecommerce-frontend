...
export default AuthenticatedAdmin(class OrderDetail extends Component {
  constructor(props) {
    super(props);
          ...
          <TableBody displayRowCheckbox={false}>
            {lineItems && lineItems.map((item, index) => {
              return <TableRow key={index}>
                ...
                <TableRowColumn><FormattedNumber value={item.price}/></TableRowColumn>
                <TableRowColumn><FormattedNumber value={item.quantity}/></TableRowColumn>
                <TableRowColumn><FormattedNumber value={item.quantity * item.price}/></TableRowColumn>
              </TableRow>
            })}
        ...
        <div className="row">
          <div className="col-md-12">
            ...
            <div className="text-right">
              <RaisedButton label="CANCEL" style={style} onTouchTap={e => this.onButtonClick(e, "CANCELLED")}/>
              <RaisedButton label="SHIPPING" primary={true} onTouchTap={e => this.onButtonClick(e, "SHIPPING")}/>
            </div>}

            {orderDetail && orderDetail.status === "SHIPPING" &&
            <div className="text-right">
              <RaisedButton label="CANCEL" style={style} onTouchTap={e => this.onButtonClick(e, "CANCELLED")}/>
              <RaisedButton label="DELIVERED" primary={true} onTouchTap={e => this.onButtonClick(e, "DELIVERED")}/>
            </div>}
          </div>
        </div>
